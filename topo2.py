...
class FatTree(Topo):
	...
	self.PodNum = self.k
        ...
        self.CoreName = ['crSw'+str(x) for x in range(self.CoreNum)]
	self.AggrName = []
	self.EdgeName = []
	self.EdgeNum = int(self.k / 2 * self.PodNum)
	#for i in range(int(self.k/2)):
		#for j in range(int(self.k/2)):
		#self.CoreName = ['crSw'+str(i)+str(x) for x in range(self.CoreNum)]
        self.AggrNum = int(self.k / 2 * self.PodNum)
        #self.AggrName = ['agSw'+str(x) for x in range(self.AggrNum)]
	for i in range(int(self.k)):
		for j in range(int(self.k/2)):
			self.Aggr = 'agSw'+str(i)+str(j)
			self.AggrName.append(self.Aggr)
			self.Edge = 'edSw'+str(i)+str(j)
			self.EdgeName.append(self.Edge)
