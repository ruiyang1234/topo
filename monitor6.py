...
class SimpleMonitor13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.datapaths = {}
        self.limit=input('thanks for your register,please input how many data do you want to buy?')
    ...
    def _monitor(self):
        while True:
            ...
            hub.sleep(2)
            if self.limit=='0':
                buy_message=input('you have gone over your data limit, do you want to buy some more data?')
                if buy_message=='no':
                    self.logger.info('thanks for using our service, your account has closed')
                    self.limit='a'
                ...
                elif buy_message=='yes':
                    buy_amount=input('how many data bits do you want to buy?')
                    print('thank you, we have add the limit for your account and below is your data info')
                    self.limit=buy_amount
                    print('your limit now is %d', self.limit)
                    out_port1=2
                    out_port2=1
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            if stat.match['in_port']==1:
                if stat.byte_count < int(self.limit):
            
                    self.logger.info('we have %016x %8x %8x %8d %8d',
                                     ev.msg.datapath.id,
                                     stat.match['in_port'],
                                     stat.instructions[0].actions[0].port,
                                     stat.packet_count, stat.byte_count)
